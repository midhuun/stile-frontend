...
const Account = () => {
  ...
  async function getOrders() {
    ...
    const res = await fetch(`https://stilebackend.vercel.app/user/orders`, {
      credentials: 'include',
      headers: {
  ...
  async function handleLogout() {
    localStorage.clear();
    await fetch('https://stilebackend.vercel.app/user/logout', {
      method: 'POST',
      credentials: 'include',
    });
    setisUserOpen(false);
  ...
  async function handleCancelOrder() {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`https://stilebackend.vercel.app/order/delete/${cancelOrderId}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
    ...
    <div className="max-w-4xl mx-auto pt-10 p-6 bg-white  rounded-lg">
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center text-gray-500 mt-10">
          <img src="/no-order.png" alt="No Orders" className="w-24 h-24 mb-4 opacity-60" />
          <p className="text-lg font-semibold">No orders yet!</p>
          <p className="text-sm mt-2">
            Looks like you haven’t placed any orders. Start shopping now and fill your cart with
            amazing products!
          </p>
          <a
            href="/shop"
            ...
            <div className="flex p-3  justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <span className="text-gray-800 text-[12px] md:text-sm">
                    {' '}
                ...
                <div className="p-3  mt-2">
                  <div className=" flex gap-3  relative">
                    ...
                    <img
                      ...
                      alt="src={order.products[0].product.name}"
                      className="w-16  h-20 object-cover"
                    />
                    <div className="py-2">
                      <p className="text-[12px] md:text-sm">{order.products[0].product.name}</p>
                      <p className="text-[12px] font-semibold md:text-sm text-black">
                        <span className="font-semibold text-gray-500 pr-1"> Size:</span>
                  ...
                  <div className="flex mt-4 justify-between ">
                    <Link to={`/order/${order.orderId}`}>
                      <button
                        ...
                        className="w-28 px-3 md:text-sm text-[12px] py-2 hover:bg-black hover:text-white border bg-white text-black duration-500 transition-all"
                      >
                        View Order
                      </button>
                    </Link>
                    <button
                      ...
                      className="w-28 px-3 py-2 text-[12px] md:text-sm hover:bg-black hover:text-white border bg-white text-black duration-500 transition-all"
                    >
                      Cancel Order
                    </button>
                  </div>
