        ...
        <div className="max-w-4xl mx-auto mt-8 p-6 bg-white shadow-md rounded-lg">
            ...
            <div className="flex justify-between items-center bg-gray-100 p-4 rounded-md mb-6">
                <span className={`text-sm font-medium px-3 py-1 rounded-full 
                    ${order.status === "Delivered" ? "bg-green-200 text-green-700" :
                        order.status === "Shipped" ? "bg-yellow-200 text-yellow-700" :
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="p-4 border rounded-lg shadow-sm">
                    ...
                    <p className="text-gray-600 text-sm">Payment Method: <strong>{order.paymentMethod?.toUpperCase()}</strong></p>
                    <p className="text-gray-600 text-sm">Total Amount: <strong>₹{order.totalAmount}</strong></p>
                </div>
            ...
            {order.address && (
                <div className="p-4 border rounded-lg shadow-sm mb-6">
                    ...
                    <p className="text-gray-600 text-sm"><strong>{order.address.name}</strong></p>
                    <p className="text-gray-600 text-sm">{order.address.Location}, {order.address.Apartment}</p>
                ...
                {order.products && order.products.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {order.products.map((item: any) => (
                            <div key={item._id} className="border rounded-lg shadow-sm p-3  bg-white hover:shadow-md transition-all duration-200">
                                <img
