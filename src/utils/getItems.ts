export async function getCart(){
    const response = await fetch('http://localhost:3000/user/cart',{credentials:'include'});
    const data = await response.json(); 
    return data.cart;
  }
export async function getFavourites(){
    const response = await fetch('http://localhost:3000/user/favourites',{credentials:'include'});
    const data = await response.json(); 
