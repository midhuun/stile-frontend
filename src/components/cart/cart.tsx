...
const CartPage = () => {
  ...
  async function paymentCheck(orderid: any) {
    const res = await fetch(`https://stile-backendd.vercel.app/payment/status/${orderid}`, {
      method: 'POST',
    });
      ...
      try {
        ...
        if (status.length > 0) {
          ...
          if (status[0].paymentStatus === 'SUCCESS') {
            ...
            // const res = await fetch("https://stile-backendd.vercel.app/user/order", {
            //       credentials: 'include',
            //       method: 'POST',
  ...
  const doPayment = async (token, orderid) => {
    ...
    await fetch('https://stile-backendd.vercel.app/user/order', {
      credentials: 'include',
      method: 'POST',
      headers: {
  ...
  async function handlePayment(e: any) {
    ...
    const res = await fetch('https://stile-backendd.vercel.app/user/payment', {
      credentials: 'include',
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
