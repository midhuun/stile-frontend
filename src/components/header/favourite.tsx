...
const Favorites = () => {
  ...
  async function getFavorites() {
    ...
    const res = await fetch('https://stile-backend-api.vercel.app/user/favourites', {
      credentials: 'include',
      headers: {
  ...
  const handleRemoveFavorite = async (item: any) => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`https://stile-backend-api.vercel.app/user/removeFromFavourites`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
