...
const Favorites = () => {
  ...
  async function getFavorites(){
    const res = await fetch('https://admin-stile-12333.vercel.app0/user/favourites',{credentials:'include'});
  ...
  const handleRemoveFavorite = async (item:any) => {
    try {
      const res = await fetch(`https://admin-stile-12333.vercel.app0/user/removeFromFavourites`, {
        method: 'POST',
        credentials: 'include',
        headers: {
    ...
    >
      {favourites && favourites.length > 0 ? (
        <div className="px-4 relative mt-5 max-h-[calc(100vh-160px)] mb-5 border-b border-gray-300 overflow-y-scroll">
          <TfiClose onClick={() => setisFavouriteOpen(false)} size={18} className="absolute cursor-pointer text-md md:text-2xl right-4 top-4" />
                ...
                <div className="flex w-2/3 gap-4">
                  ...
                  <div className="desc w-full flex flex-col justify-between h-full text-gray-700 text-sm">
                    <div>
                      ...
                      <h1 className="text-sm md:text-md font-semibold cursor-pointer hover:underline">{item.name.slice(0, 15)}{item.name.length > 15 && "..."}</h1>
                      </Link>
                      <p className="text-gray-700 text-sm md:text-md font-light">Rs.{item.price.toFixed(2)}</p>
                    </div>
                  </div>
                </div>
                <button onClick={() => handleRemoveFavorite(item)} className="mx-3 text-lg transition duration-200"><RiDeleteBinLine /></button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-5 px-4 relative">
          <TfiClose onClick={() => setisFavouriteOpen(false)} size={16} className="absolute cursor-pointer right-4 top-1" />
