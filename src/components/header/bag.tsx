        ...
        >
          ...
          <div className="flex pt-2 justify-between text-black">
            ...
            <p>
              Rs.
              {cart
                .reduce((total: any, item: any) => total + item?.product?.price * item?.quantity, 0)
                .toFixed(2)}
            </p>
          </div>
          ...
          <Link to="/checkout">
            <button
              onClick={() => setiscartOpen(false)}
              className="bg-black fixed bottom-3 left-1/2 -translate-x-1/2 text-white w-[90%] transform  py-2 rounded-lg transition duration-200 hover:bg-gray-800"
            >
              Checkout
            </button>
          </Link>
