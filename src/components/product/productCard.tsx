    ...
    <div className="h-[350px] flex flex-col  w-[140px] md:h-[560px] md:w-[350px]" key={product?._id}>
      <div className='overflow-hidden relative h-[200px] md:h-[450px]'>
        ...
        <div className="md:p-2 py-2 ">  
            <p className="text-[12px] tracking-widest font-medium h-10 md:text-[16px] uppercase">{product?.name}</p>
            <p className="text-[10px] tracking-wide text-gray-700 h-10 md:h-5 md:text-[13px] ">{product?.description.split(" ").slice(0,6).join(" ")}</p>
            <p className="text-[12px] mt-1 md:text-sm text-black tracking-widest">Rs. {(product?.discountedPrice!==product.price)?product?.discountedPrice:product?.price} <span className="pl-1 md:pl-3 text-[10px] text-gray-400 line-through">{product?.discountedPrice && product?.discountedPrice!==product.price && `Rs. ${product.price}`}</span></p>  
            </div>
    </div>
    </Link>
  )
