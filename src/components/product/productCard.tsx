...
const ProductCard = ({product}:any) => {
  ...
  return (
    <div className={`h-[350px] flex flex-col ${product.type === 'home' ? 'w-[140px]' :"w-[49%] xs:w-[32%] sm:w-[30%]" } h-[350px]  md:h-[560px] md:w-[300px]`} key={product?._id}>
       <Link to={`/product/${product.slug}`}>
      <div className='overflow-hidden relative h-[280px] md:h-[450px]'>       
        ...
        <div className="md:p-2 py-2 ">  
          <div className='h-10 overflow-hidden flex items-center'>
            <p className="text-[12px]  font-semibold  md:text-[16px] uppercase break-words">{product?.name}</p>
            </div>
