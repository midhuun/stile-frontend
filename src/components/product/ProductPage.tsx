...
const ProductPage = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const data = await fetch('https://stilebackend.vercel.app/reviews', {
        method: 'POST',
        headers: {
  ...
  const addToFavorite = async () => {
    ...
    try {
      ...
      await fetch(`https://stilebackend.vercel.app/user/addtoFavourites`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleCart = async (value: any) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`https://stilebackend.vercel.app/user/${value}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
      ...
      try {
        if (productdata) {
          const response = await fetch(
            `https://stilebackend.vercel.app/reviews/${productdata._id}`
          );
  ...
  useEffect(() => {
    const getProduct = async () => {
      try {
        const response = await fetch(`https://stilebackend.vercel.app/product/${product}`);
        const data = await response?.json();
        if (data.length > 0) {
