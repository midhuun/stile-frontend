...
const ProductPage = () => {
  ...
  const { setiscartOpen, setisFavouriteOpen, setFavourites, isAuthenticated, setisUserOpen, user } =
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const data = await fetch('https://stile-backend-rvmk.vercel.app/reviews', {
        method: 'POST',
        headers: {
  ...
  const thisProduct =
    productdata && cart
      ? cart?.find(
          (item: any) => item.product._id === productdata?._id && item.selectedSize === activeSize
        )
      : null;
  ...
  const addToFavorite = async () => {
    ...
    try {
      ...
      await fetch(`https://stile-backend-rvmk.vercel.app/user/addtoFavourites`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleCart = async (value: any) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`https://stile-backend-rvmk.vercel.app/user/${value}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
      ...
      getCart()
        .then((item: any) => dispatch(setcart(item)))
      ...
      try {
        if (productdata) {
          const response = await fetch(
            `https://stile-backend-rvmk.vercel.app/reviews/${productdata._id}`
          );
          ...
          if (data?.averageRating) {
            ...
            setReviewsList([
              ...
              },
              {
                ...
                rating: 4.5,
                content:
                  'The material is really good, and it feels comfortable. Delivery was quick too. Would definitely buy again!',
              },
              {
                name: 'Saran',
                ...
                rating: 4.8,
                content:
                  "I was skeptical at first, but this product is fantastic! Using it for a week now, and it's been perfect.",
              },
              {
                name: 'Praveen',
              ...
              },
              {
                ...
                rating: 5,
                content:
                  'The best product I’ve bought in a long time! Works exactly as described. Highly recommend to everyone.',
              },
            ]);
          }
        }
  ...
  useEffect(() => {
    const getProduct = async () => {
      try {
        const response = await fetch(`https://stile-backend-rvmk.vercel.app/product/${product}`);
        const data = await response?.json();
        if (data.length > 0) {
    ...
    <>
          ...
          <div className="space-y-2 p-2 md:w-[50%]">
            ...
            <div className="flex items-center gap-3">
              <p className="rating p-1 md:p-2 text-[12px] select-none md:text-xs w-14 rounded-md flex text-white font-bold justify-center items-center gap-1 bg-green-500">
                ...
                <span className="tracking-widest">{Math.floor(avg * 10) / 10}</span>
              </p>
            ...
            <h2 className="text-md md:text-xl font-bold">
              ...
              <span className=" font-normal text-[11px] md:text-xs text-gray-400">
            ...
            {/* Size Chart */}
            <div>
              <button onClick={() => setchartOpen(true)} className="flex items-center gap-2">
                ...
                <div className="fixed inset-0 top-10 flex items-center   justify-center bg-black bg-opacity-50 z-50">
                  <div className="bg-white rounded-lg p-6 relative">
                    ...
                    <div className="flex w-full justify-center items-center">
                      <img
                        src={productdata?.subcategory?.sizeurl}
                        className="min-w-[300px]  h-[60vh]"
                        alt="Size Chart Icon"
                      />
                    </div>
                  </div>
            ...
            <p className="text-xs md:text-sm">
              <span className="font-semibold text-xs md:text-sm">Note:</span> Please check size
              before buying
            </p>
            {/* Size Selection */}
            <div>
                      ...
                      {key === 'shipping' && (
                        <div className="p-4 bg-gray-100 rounded-lg shadow-md">
                          ...
                          <ul className="list-disc list-inside space-y-1 text-xs md:text-sm">
                            <li>
                              <strong>Shipping Methods:</strong> We usually ship with private
                              couriers through Shiprocket, ST Couriers, Professional Courier, DTDC,
                              etc. For other states, we use Blue Dart, Speed Post, Ekart, Ecomm
                              Express, and Delhivery.
                            </li>
                            <li>
                              <strong>Dispatch Time:</strong> Dispatch time is typically within
                              48-72 hours.
                            </li>
                            <li>
                              <strong>Delivery Time:</strong> Delivery time within India is
                              typically 4-7 business days, while international deliveries take
                              around 7-12 business days.
                            </li>
                            <li>
                              <strong>Note:</strong> At times, delivery may take longer due to
                              changes in delivery routes or other unforeseen circumstances.
                            </li>
                            <li>
                              <strong>Legal Disputes:</strong> All legal disputes are subject to the
                              jurisdiction of Tirupur.
                            </li>
                          </ul>

                          {/* Returns & Exchange Section */}
                          <h3 className="font-semibold text-base md:text-lg my-2">
                            Cancellation / Refund Policy
                          </h3>
                          <ul className="list-disc list-inside space-y-1 text-xs md:text-sm">
                            <li>
                              <strong>Return Policy:</strong> We have a 7-day return policy, which
                              means you have 7 days after receiving your item to request a return.
                            </li>
                            <li>
                              <strong>Eligibility for Return:</strong>
                              <ul>
                                <li>
        ...
        <div className="box h-auto w-full p-6 shadow-lg rounded-lg md:w-[80%] bg-white text-gray-900">
          ...
          <div className="rating my-4 p-4 flex flex-col md:flex-row items-center bg-gray-100 rounded-lg w-full shadow-sm">
            <div className="total p-6 flex justify-center items-center gap-4 flex-col md:w-1/3">
              <h1 className="md:text-[70px] text-5xl text-gray-600 font-extrabold">
                {Math.floor(avg * 10) / 10}
              </h1>
            ...
            <form onSubmit={handleSubmit} className="w-full md:w-2/3 p-4 flex flex-col gap-4">
              ...
              <input
                ...
                id=""
                className="p-2 text-sm border-gray-300 md:text-md w-full border rounded-lg"
              />
              <input
                value={review.title}
                onChange={(e) => handleReview(e)}
      ...
      <div className="reviews-list px-6 md:px-6 mt-6 w-full grid grid-cols-1 md:grid-cols-4 gap-3 md:gap-4">
        {reviewsList.map((rev: any, index: number) => (
          <div
            key={index}
            className="p-4 bg-white rounded-lg shadow-lg border border-gray-200 flex flex-col md:flex-row items-start gap-4"
          >
            {/* User Avatar */}
              ...
              <div className="flex items-center  justify-between">
                ...
                <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    <FaStar
                      key={i}
                      className={`${i < rev.rating ? 'text-yellow-400' : 'text-gray-300'} text-md`}
                    />
                  ))}
                </div>
              </div>
